...
export default function ProductDetailPage() {
  ...
  const catName = cat ? (lang === 'ar' ? cat.nameAr : cat.name) : ''

  const similar = data.products.filter(p => p.category === product.category && p.id !== product.id).slice(0, 4)

  return (
    <PageTransition>
      <Helmet>
        <title>{name} — Mawada Parapharmacie</title>
      ...
      <div className="pt-24 bg-ivory min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <FadeIn className={`flex items-center gap-2 mb-8 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <span className="text-rose-200" aria-hidden="true">/</span>
            <span className="text-sm text-charcoal font-sans truncate max-w-40">{name}</span>
          </FadeIn>
          ...
          <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16 ${isRTL ? 'lg:flex lg:flex-row-reverse' : ''}`}>
            ...
            <FadeIn direction={isRTL ? 'left' : 'right'} delay={0.1} className={isRTL ? 'text-right' : ''}>
              ...
              {/* Name */}
              <h1 className="font-serif text-3xl sm:text-4xl text-charcoal mb-4 leading-tight">{name}</h1>
              ...
              {/* Description */}
              <p className="text-base text-charcoal-light font-sans leading-relaxed mb-6">{description}</p>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <a
                  href={buildWhatsAppUrl(product.whatsappText, lang)}
                  target="_blank"
                ...
                <a
                  ...
                  aria-label={`Commander ${name} via Messenger`}
                >
